    ...
    public override void Heuristic(in ActionBuffers actionsOut)
{
        ...
        // Overriding behaviour for the NN. 
        GameObject target = GetNearestTarget();
        // Ball carrying Strategy: always pursue balls. 
        if (GetCarrying() < 2 && target != null) {            
            GoToNearestTarget();
            return;
        }
        // Ball carrying Strategy: if more tham two balls are carried, player must go deposit back to base. 
        if (GetCarrying() >= 2) {
            GoToBase();
            return;
        }
        int goToBaseAxis = (int)actions.DiscreteActions[4]; 
        // Penalize for time, in order to minimize wasted movement. Improves efficiency 
        AddReward(-1.0f);
        // Move player
...
// ----------------------ONTRIGGER AND ONCOLLISION FUNCTIONS------------------------
    ...
    protected override void OnTriggerEnter(Collider collision)
    {
        ...
        {
            // Reward for bringing back targets 
        ...
        {
            // penalize for staying in the base while not carrying any balls. 
            AddReward(-0.3f);
        }
    ...
    protected override void OnCollisionEnter(Collision collision)
    {
        ...
        // if the target hits a wall
        if (collision.gameObject.CompareTag("Wall"))
        {
            //penalize to avoid hitting the wall 
            AddReward(-1.0f);
        }
    ...
    // Helper function that returns true if enemy is within a given range of the player
